...
if ( count($errors) == 0 ) {
  // no errors, insert the image
  $query = sprintf(
    "insert into movie (filename, mime_type, file_size, file_data) values ('%s', '%s', '%d', '%s'",
  ...
  // redirect the user to view the new image
...
<body>
  ...
  <ul>
    <?php for each ( $errors as $error ) {?>
      <li>
        <?php echo htmlSpecialChatacters($error) ?>
      </li>
    <?php } ?>
  </ul>
  <p>
